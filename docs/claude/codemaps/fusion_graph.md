...
- The stuck-value gate's accumulators (`gps_stuck_gate.hpp`) do the OPPOSITE on purpose: they reset ONLY when the reported lat/lon actually changes, and must keep growing across repeated rejections of the same stuck value — that is the entire mechanism. Do not "fix" this by resetting them every fix like the wrongfix gate; that would make a genuinely stuck receiver undetectable again.
