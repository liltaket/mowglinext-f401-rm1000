...
| Upstream branch | `dev` |
| Upstream commit | `c21b18c55cc71bb010a7f0c70d42ad896a53304d` |
...
commit `5fae125b77b16b2472b48a3213d4d4f1c9d93f0a` initializes the handle before
...
Clean fix commit `e84e4de60d99fd4bfde55b51ca973a2c59945ce8`
...
measurement. Yardforce500B, Yardforce500 and the coast-down validation build,
native tests, current firmware guards, reversal replay and sensor-recovery
tests passed.

Upstream PR [#559](https://github.com/mowglinext/mowglinext/pull/559) merged
after the original investigation and is included in the refreshed baseline.
The port preserves its direction checksum, accepted-OFF dwell, fresh zero-epoch
and held-speed-word guards. A deterministic regression also covers a reply
arriving after reversal qualification but immediately before DMA submission:
the final masked handoff binds the ON frame to the exact qualifying reply and
downgrades it to checksum-correct OFF if that evidence changed. Reported zero
RPM still does not prove that the physical rotor has stopped.
...
input mode. Commit `ac919cb41aa83e61f45870e7546bbf168b2b476f` selects
...
`f6ea782aa7b8e400c0e0af53e5adbf56e8818bde` enables it before GPIO
...
| --- | --- | --- | --- | --- | --- |
| F401 RTC | Uninitialized handle and unchecked backup payload | Initialize/order/close backup access; mark and range-check charge state | `5fae125b` | F401 + F103 pass | No |
| Motor RX/UART | Raw DMA consumption, no freshness gate, weak recovery | Validated snapshots, age/error gates, shared sticky inhibit, explicit zero re-arm, bounded RX/TX recovery, reversal evidence bound at DMA handoff | `e84e4de6` | F401 + F103 + validation + native pass | No |
| F401 panel RX | AF7 selected while pin remains GPIO input | F401 RX uses AF push-pull; F103 unchanged | `ac919cb4` | F401 + F103 pass | No |
| ADC GPIOC clock | Clock not enabled in ADC init path | Enable GPIOC before analog-pin configuration | `f6ea782a` | F401 + F103 pass | No |
...
9. Verify blade communication with the cutting blade removed and independent
   rotor-motion observation. Interrupt the zero-confirmation interval and inject
   a new active/nonzero reply immediately before reversal handoff; verify OFF
   persists. Controller-reported zero RPM and the 1000 ms dwell are not by
   themselves a physical-stop pass criterion.
