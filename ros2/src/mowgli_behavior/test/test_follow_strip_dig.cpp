...
TEST_F(FollowStripDigTest, ScanPauseSurvivesABladeOffTransitToTheNextUnit)
{
  ...
  ASSERT_EQ(tickUntil(
                [&]()
                {
                  // This is the handoff tick. Do not let asynchronous follow
                  // goal delivery add ordinary fresh-scan ticks before the
                  // assertion below.
                  return ctx->coverage_scan_paused && ctx->transiting;
                },
                3.0),
            BT::NodeStatus::RUNNING);
  EXPECT_TRUE(ctx->coverage_scan_paused);
