 ...
 * without a transit or progress mutation. FollowStrip is ticked exactly as the
 * tree ticks it.
 */
...
TEST_F(FollowStripDigTest, ScanPauseSurvivesABladeOffTransitToTheNextUnit)
{
  ...
  // Finish unit one while scans remain stale. The structural boundary transit
  // and the second follow goal must retain both the overlay and blade-off state.
  follow->succeed(0);
  ASSERT_EQ(tickUntil(
                [&]()
                {
  ...
  for (std::size_t i = first_blade_off; i < bladeRequestCount(); ++i)
  {
    EXPECT_EQ(bladeRequest(i).mow_enabled, 0u)
        << "the blade must stay off across the stale-scan unit boundary";
  }
