...
- Make sure you Build and Upload the env that matches your mainboard: `Yardforce500` (STM32F103VC, the `default_envs`) or `Yardforce500B` (STM32F401VC). `BiltemaRM1000` inherits the `Yardforce500B` hardware profile and explicitly selects the Yardforce 900 ECO panel plus the RM-MB V6.1 PAC5223 power/start sequence and command values (`0x81` forward, `0xc1` reverse, `0x02` stop). Generic 500B retains its existing blade-controller behavior until that hardware has been independently verified. `BiltemaRM1000_MPU6050_Yaw180` additionally rotates external IMU vectors 180 degrees around Z for Bruno's physical MPU-6050 mounting; generic Yardforce500B and RM1000 builds retain the identity transform. The `Yardforce500_STLINK_V3` / `Yardforce500B_STLINK_V3` variants upload through OpenOCD instead of PlatformIO's built-in ST-Link protocol. From a shell that is e.g. `pio run -e Yardforce500 -t upload`.
