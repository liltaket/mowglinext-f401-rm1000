...
#define I2C_SENSOR_TIMEOUT_MS 10u
...
{
  reg |= 0x80;
  return HAL_I2C_Mem_Write(handle, LIS3DH_I2C_ADD_L, reg,
                           I2C_MEMADD_SIZE_8BIT, (uint8_t*) bufp, len,
                           I2C_SENSOR_TIMEOUT_MS) == HAL_OK ? 0 : -1;
}
...
int32_t I2C_platform_read(void *handle, uint8_t reg, uint8_t *bufp, uint16_t len)
{
  ...
  reg |= 0x80;
  return HAL_I2C_Mem_Read(handle, LIS3DH_I2C_ADD_L, reg,
...
uint8_t I2C_TestZLowINT(void)
{
    ...
    if (lis3dh_int1_gen_source_get(&dev_ctx, &int1_src) != 0) {
        /* A missing safety sensor must not be interpreted as "safe". */
        return 1u;
    }
