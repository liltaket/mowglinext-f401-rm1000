...
static volatile uint8_t emergency_physical_active = 0;
#else
/* Fail closed until the foreground has completed its first sensor poll. */
static volatile uint8_t emergency_physical_active = 1;
#endif
...
{
    return emergency_physical_active;
}
...
void EmergencyController(void)
{
    ...
    /* Publish one atomic, read-only snapshot for USB callbacks. Calling the
     * sensor functions from a priority-0 USB IRQ used to enter blocking I2C;
     * on STM32F4 that can prevent HAL_GetTick() from advancing and hang until
     * a watchdog reset. */
    emergency_physical_active =
        (stop_button_yellow || stop_button_white || wheel_lift_blue ||
         wheel_lift_red || tilt || accelerometer_int_triggered) ? 1u : 0u;
    ...
    if (emergency_state && play_button && !emergency_physical_active)
    {
